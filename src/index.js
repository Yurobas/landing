document.addEventListener("DOMContentLoaded", () => {
      ...
      if (checkScreenSize() > 1279) {
        ...
        if (pageYOffset >= calc.top && pageYOffset <= calc.bottom) {
          requestAnimationFrame(() => {
            requestAnimationFrame(() => {
              if (checkScreenSize() < 1280) {
                bg.el.style.transform = `translate3d(-50%, calc(-50% + ${bg.max * mod}px), 1px)`;
              }
            })
          })
        }
  ...
  +function forms() {
    ...
    forms.forEach(form => {
      ...
      email.input.addEventListener('input', () => {
        if (email.input.value.length > 0) {
          ...
          email.input.classList.remove('--value');
        }
      })
    ...
    function validation(name, phone, email, policy) {
      ...
      if (name.test() && phone.test() && email.test() && policy.checked) {
        check = true; 
      }
  ...
  +function statsCounters() {
    const values = [...document.querySelectorAll('[data-stats-value]')];
    values.forEach(item => {
      let min, max;
      const type = item.dataset.statsValue;
      const elements = [...item.querySelectorAll('.stats__item-value')];
      ...
      switch(type) {
        case 'clients':
          ...
          if (!localStorage.getItem('clients')) {
            localStorage.setItem('clients', value);
          } else {
            if (localStorage.getItem('clients') != value) {
        ...
        case 'bottles':
          min = 2;
          max = 6;
          if (!localStorage.getItem('bottles')) {
            localStorage.setItem('bottles', value);
          } else {
            if (localStorage.getItem('bottles') != value) {
